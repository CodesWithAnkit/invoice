...
- `[x]` **MVP Phase 0 — Security & ownership** (2026-09-26). Business per user (trigger), `business_id` + RLS on all legacy tables, every API route session-guarded and business-scoped, browser → table access removed (new `/api/invoices`, `/api/invoices/:id`, `/api/customers`), server-side invoice totals, D1 duplicate-customer fix, private PDF bucket, template publishing admin-only, `claim_legacy_data` for existing rows (owner: webans001@gmail.com).
  - **Phase gate:** full `npm run test:e2e` → 254 passed / 0 failed / 20 skipped (pre-existing), including the new backend `tests/api/ownership.spec.ts` and frontend `tests/ownership.spec.ts`. `next build` passes.
  - `tests/invoice-view.spec.ts`: the mock now targets `/api/invoices/test-id` instead of Supabase REST (the page no longer calls REST); the assertions are unchanged.
  - **Hosted rollout pending:** `docs/runbooks/phase0_hosted_rollout.md` (migration → claim → deploy, in one window).
